...
increment_active_requests (xsp_data *conf, request_rec *r, int32_t id)
{
	...
	if (max_active_requests > 0 && conf->dashboard->active_requests >= max_active_requests) {
		...
		{
			int i;
			for (i = 0; i < WAITING_URI_LIST_ITEM_COUNT; i++) {
				if (conf->dashboard->waiting_uri_list[i].id == id) {
					conf->dashboard->waiting_uri_list[i].id = -1;
					conf->dashboard->waiting_uri_list[i].start_time = -1;
					break;
				}
			}
		}
 ...
 static void
decrement_active_requests (xsp_data *conf, uint32_t id)
{
#ifndef APACHE13
	apr_status_t rv;
...
mono_execute_request (request_rec *r, char auto_app)
{
	...
	if (conf->dashboard_mutex && conf->dashboard) {
		if (apr_global_mutex_lock (conf->dashboard_mutex) == APR_SUCCESS) {
			...
			if (ok) {
				id = conf->dashboard->requests_counter++;
